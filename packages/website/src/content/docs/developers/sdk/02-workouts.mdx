...
`reps: [5, 5, 3]`. State `primaryUnit` for the `reps` slot and `secondaryUnit` for the
`weight` slot. `buildSession` checks both against the exercise library before writing.
