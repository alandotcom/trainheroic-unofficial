...
describe("ExerciseStore safety + write-through", () => {
  ...
  it("update POSTs then write-throughs the renamed row", async () => {
    ...
    const store = newStore();
    await store.refresh();
    const updated = await store.update(555, { title: "Renamed", param_1_type: 3 });
    expect(updated.title).toBe("Renamed");
    expect((await store.get(555))?.title).toBe("Renamed");
    const posted = vi
      .mocked(fetch)
      .mock.calls.filter(
        ([url, init]) =>
  ...
  it("remove DELETEs the live exercise then drops the D1 row", async () => {
    mockApi([{ id: 999, title: "Custom Move", param_1_type: 3, can_edit: 1 }]);
    const store = newStore();
    await store.refresh();
    ...
    const deleted = vi
      ...
      .mock.calls.filter(
        ([url, init]) =>
          String(url).includes("/v5/exercises/999") &&
