...
// and blocks `--yes` so a read eval can never commit a write. The role only changes the prompt
// preamble and the canonical command map — the CLI itself is one binary with both command groups.
...
function makeRunOnce(role: Role) {
  const normalize: Normalize = (rawName, input) => {
    ...
    return normalizeCliCommand(input.command, role);
  };
  ...
  return async function runOnce(
    ...
    const dir = await mkdtemp(join(tmpdir(), `th-eval-cli-${role}-`));
...
export function cliDriver(role: Role): Driver {
  return { surface: "cli", runOnce: makeRunOnce(role) };
}
