...
export class ExerciseStore extends OrgScopedStore implements ExerciseIndex {
  ...
  async #isStale(org: number): Promise<boolean> {
    const last = await this.#meta(org, "last_full_sync");
    if (last === null) return true;
    return Date.now() - Number(last) > TTL_MS;
  }
  ...
  async ensureFresh(force = false): Promise<void> {
    ...
    if (force || (await this.#count(org)) === 0 || (await this.#isStale(org))) {
      await this.refresh();
    }
  }
  ...
  async recordUpsert(ex: Record<string, unknown>): Promise<void> {
    const org = await this.org();
    const generation = Number((await this.#meta(org, "sync_generation")) ?? "0");
  ...
  async #searchOnly(query: string, limit: number): Promise<ExerciseView[]> {
    ...
    const binds = [org, ...tokens.map((t) => `%${t}%`), limit * 3];
    const res = await this.db
      .prepare(
        `${SELECT_CORE} WHERE org_id = ? AND ${where} ORDER BY use_count DESC, length(title) ASC LIMIT ?`,
      )
      .bind(...binds)
      .all<ExerciseRow>();
